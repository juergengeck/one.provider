...
export default class ChannelManager {
    ...
    private readonly instanceId: string = Math.random().toString(36).substring(2, 10);
        ...
        try {
            ...
            try {
                try {
                    ...
                    try {
                        const storedVersion = await getCurrentVersion<ChannelInfo>(channelInfoIdHash);
    ...
    private async addChannelIfNotExist(
        ...
        try {
            ...
            try {
                const existsInStorage = await getObjectByIdHash(channelInfoIdHash);
    ...
    private async loadRegistryCacheFromOne(): Promise<void> {
        ...
        await serializeWithType(ChannelManager.registryLockName, async () => {
            ...
            await Promise.all(
                [...registry.channels].map(async channelIdHash => {
                    try {
                        ...
                        const channelInfoResult = await getObjectByIdHash(channelIdHash);
                        ...
                        if (channelHead) {
                            try {
                                const headEntry = await getObject<LinkedListEntry>(channelHead);
            ...
            const channelsByType = new Map<string, number>();
            for (const channelInfo of this.channelInfoCache.values()) {
     ...
     * @private
     */
    private async countChannelEntries(headHash: SHA256Hash<LinkedListEntry>): Promise<number> {
        let count = 0;
        let currentHash: SHA256Hash<LinkedListEntry> | undefined = headHash;
        
        while (currentHash) {
            count++;
            try {
                const currentEntry: LinkedListEntry = await getObject<LinkedListEntry>(currentHash);
